...
function ParametrizedCaseViewer ({ clients, user, app, config }: {
  ...
  const scanFileName = params.scanFileName
...
class SlimViewer extends React.Component<SlimViewerProps, SlimViewerState> {
  ...
  constructor (props: SlimViewerProps) {
    ...
    if (oidcSettings !== undefined) {
      ...
      // Use the current full URL as redirect URI to preserve the path
      const currentUrl = window.location.origin + window.location.pathname
      this.auth = new OidcManager(currentUrl, oidcSettings)
    }
  ...
  render (): React.ReactNode {
    const appInfo = {
      name: this.props.name || 'slim',
      version: this.props.version || '0.42.4',
      homepage: this.props.homepage || 'https://github.com/ImagingDataCommons/slim',
    ...
    const viewerHeight = this.props.height || '100vh'
    const layoutStyle = { height: viewerHeight }
    ...
    if (this.state.redirectTo !== undefined) {
      ...
      return (
        <div 
          ...
          style={{ ...this.props.style, height: viewerHeight }}
        >
